<script lang="ts">
	...
	const slug = $derived(href !== '#' ? href.replace(/\//g, '-').replace(/^-/, '') : '');
	const imgStyle = $derived(slug ? `view-transition-name: ${slug}` : undefined);
	const titleStyle = $derived(slug ? `view-transition-name: ${slug}-title` : undefined);
	const descStyle = $derived(slug ? `view-transition-name: ${slug}-desc` : undefined);
</script>
		...
		<div class="card-body p-8">
			<div class="mb-4 flex items-start justify-between">
				<div>
					...
					<h3 class="font-serif text-[40px] font-semibold leading-[48px] text-forest-evergreen" style={titleStyle}>{title}</h3>
				</div>
				{#if icon}
					<Icon name={icon} class="text-forest-evergreen" />
				{/if}
			</div>
			<p class="mb-6 max-w-xl text-[16px] leading-6 text-on-surface-variant" style={descStyle}>{description}</p>
...
{:else if variant === 'portrait'}
	...
	<div class="card group flex flex-col overflow-hidden border border-outline-variant {bg} transition-all hover:shadow-lg">
		...
		<div class="card-body flex-grow p-6">
			<span class="mb-2 block text-[14px] font-semibold uppercase leading-5 tracking-[0.1em] text-harvest-oak">{category}</span>
			<h3 class="mb-4 font-serif text-[28px] font-semibold leading-9 text-forest-evergreen" style={titleStyle}>{title}</h3>
			<p class="mb-6 text-[16px] leading-6 text-on-surface-variant" style={descStyle}>{description}</p>
			<a href={href} class="link inline-flex items-center gap-2 text-[14px] font-semibold uppercase tracking-[0.1em] text-forest-evergreen no-underline transition-colors hover:text-harvest-oak">
				VIEW PROJECT <Icon name="arrow_right_alt" class="text-sm" />
			</a>
		</div>
	...
	<div class="card group relative overflow-hidden border border-outline-variant {bg} transition-all hover:shadow-lg">
		<div class="flex h-full flex-col md:flex-row">
			...
			<div class="card-body flex w-full flex-col justify-center p-8 md:w-1/2">
				<span class="mb-2 block text-[14px] font-semibold uppercase leading-5 tracking-[0.1em] text-harvest-oak">{category}</span>
				<h3 class="mb-4 font-serif text-[28px] font-semibold leading-9 text-forest-evergreen" style={titleStyle}>{title}</h3>
				<p class="mb-8 text-[16px] leading-6 text-on-surface-variant" style={descStyle}>{description}</p>
